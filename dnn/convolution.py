...
class Convolution2d(Function):
    ...
    def forward(self, X):
        ...
        for (i, j), _ in np.ndenumerate(out[0, 0]):
            x0_slice = slice((i * S0), (i * S0 + K0))
            x1_slice = slice((j * S1), (j * S1 + K1))
    ...
    def _backward(self, X, f_X, dy, update):
        ...
        for (i, j), _ in np.ndenumerate(dy[0, 0]):
            x0_slice = slice((i * S0), (i * S0 + K0)) # Start and stop points for convolutional window
            x1_slice = slice((j * S1), (j * S1 + K1)) # Start and stop points for convolutional window

            dy_expanded = np.expand_dims(dy[:, :, i:i+1, j:j+1], 2) # Adding a channel in dimension
